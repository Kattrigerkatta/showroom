if Config.useBuySystem and Config.useESX then
	ESX = nil
...
function setupScaleform(scaleform)
	...
	if Config.locations[current.ind].cars[current.vehlist[current.carInd]].desc.buyable and Config.useBuySystem then
		PushScaleformMovieFunction(scaleform, "SET_DATA_SLOT")
		PushScaleformMovieFunctionParameterInt(0)
		Button(GetControlInstructionalButton(2, 29, true))
		ButtonMessage(_("buy"))
		PopScaleformMovieFunctionVoid()
	end	
...
Citizen.CreateThread(function()
	while true do
		if not inShowRoom then
			...
			elseif IsDisabledControlJustReleased(0, 33) then -- OPEN TEBEX LINk
				if Config.locations[current.ind].cars[current.vehlist[current.carInd]].shop ~= nil then
					SendNUIMessage({
						link = Config.locations[current.ind].cars[current.vehlist[current.carInd]].shop,
					})
				end
			elseif IsDisabledControlJustReleased(0, 29) then -- Buy vehicle
				if Config.useBuySystem then
					if Config.useESX then
						-- ESX EXAMPLE --
						ESX.TriggerServerCallback('showcase:Buyvehicle', function(worked)
							if worked then
								ESX.Game.SpawnVehicle(Config.locations[current.ind].cars[current.vehlist[current.carInd]].spawnname, Config.locations[current.ind].vehicleSpawn, function(vehicle)
									TaskWarpPedIntoVehicle(PlayerPedId(), vehicle, -1)
								end)
							else	

							end
						end, current.ind, current.vehlist[current.carInd])
					end
				end
			end
		end
